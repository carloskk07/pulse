...
-- Runtime-verifiable least-privilege contract. It is intentionally readable only
-- through service_role so the public API does not disclose security internals.
...
as $$
  select jsonb_build_object(
    ...
    'authenticated_claim_rpc_execute', has_function_privilege('authenticated', 'public.claim_daily_pulse(uuid)', 'EXECUTE'),
    'service_claim_rpc_execute', has_function_privilege('service_role', 'public.claim_daily_pulse(uuid)', 'EXECUTE'),
    'claim_security_definer', (select p.prosecdef from pg_catalog.pg_proc p where p.oid = 'public.claim_daily_pulse(uuid)'::regprocedure),
