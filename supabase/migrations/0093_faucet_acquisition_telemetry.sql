-- Separate Faucet acquisition from Home acquisition and expose the distinction in
-- the private growth snapshot. This migration is intentionally outside the
-- canonical financial release marker and changes no reward or payout authority.
...
acquisition as (
  select
    ...
    count(distinct session_hash) filter (where event_type = 'faucet_view') as faucet_sessions,
...
select jsonb_build_object(
  ...
  'faucet_sessions', coalesce(a.faucet_sessions, 0),
...
revoke all on function public.admin_marketing_funnel_snapshot_by_version(integer, text)
  from public, anon, authenticated;
grant execute on function public.admin_marketing_funnel_snapshot_by_version(integer, text)
  to service_role;

comment on function public.admin_marketing_funnel_snapshot_by_version(integer, text) is
  'Service-role-only acquisition + attributed activation snapshot with distinct Home, Faucet and Proof entry measurements. Product activation remains derived from authoritative product tables.';
