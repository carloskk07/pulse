-- Pulse Direct v1 hardening
-- Makes operator funding idempotent and prevents reserved sessions from oversubscribing
-- either advertiser funding or the campaign completion cap.

create unique index if not exists direct_campaign_funding_reference_unique_idx
  on public.direct_campaign_funding(funding_reference);
...
declare
  ...
  v_existing public.direct_campaign_funding%rowtype;
  v_reference text := trim(coalesce(p_funding_reference, ''));
begin
  if p_campaign_id is null or p_amount_usd_micros is null or p_amount_usd_micros <= 0 or v_reference = '' then
    return jsonb_build_object('status', 'invalid_request');
  end if;

  perform pg_advisory_xact_lock(hashtextextended('direct-funding:' || v_reference, 0));

  select * into v_existing
  from public.direct_campaign_funding
  where funding_reference = v_reference
  limit 1;

  if found then
    if v_existing.campaign_id = p_campaign_id and v_existing.amount_usd_micros = p_amount_usd_micros then
      return jsonb_build_object(
        'status', 'idempotent',
        'campaign_id', p_campaign_id,
        'funding_id', v_existing.id,
        'amount_usd_micros', v_existing.amount_usd_micros
      );
    end if;
    return jsonb_build_object('status', 'funding_reference_conflict');
  end if;
  ...
  values (p_campaign_id, p_amount_usd_micros, v_reference);
...
begin
  ...
  if v_available < v_campaign.price_per_action_usd_micros then
    -- Reserved sessions may still expire, so this is temporary capacity pressure,
    -- not proof that the campaign's total prefunding is permanently exhausted.
    return jsonb_build_object('status', 'capacity_reserved');
  end if;
...
begin
  ...
  if v_next_count >= v_campaign.max_completions or v_unspent_total < v_campaign.price_per_action_usd_micros then
...
values (
  'release_schema',
  jsonb_build_object('version', 15, 'migration', '0015_pulse_direct_hardening.sql'),
  15,
  'Pulse Direct idempotent funding and reservation-cap hardening'
)
