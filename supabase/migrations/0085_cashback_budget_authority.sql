-- V13 cashback budget authority.
-- Cashback is feature-gated, capped by the configured 75% share, and economically immutable after creation.
...
begin
  ...
  -- The current product contract is 75%. Raising it requires a reviewed migration,
  -- not a caller/config-only change.
  ...
  -- Disabling cashback blocks creation/confirmation, but reversals remain allowed
  -- so existing liabilities can always be unwound.
...
select
  ...
  and position(
    'cashback_disabled'
    in pg_get_functiondef('public.apply_cashback_event(text,text,uuid,text,bigint,bigint,jsonb)'::regprocedure)
  ) > 0
  and position(
...
begin
  if not public.release_cashback_budget_contract() then
    raise exception 'cashback budget contract failed';
  end if;
  ...
  if coalesce((
    ...
    raise exception 'cashback budget authority requires canonical release schema v55';
  end if;
end
$$;
