-- Causal binding between FaucetPay webhook events and concrete payout dispatch attempts.
-- Webhook reconciliation remains disabled; this only hardens future matching.
-- Canonical release schema remains v55/0055.

create table if not exists public.withdrawal_dispatch_attempt_events (
  withdrawal_id uuid not null references public.withdrawals(id) on delete restrict,
  attempt integer not null check (attempt > 0),
  claimed_at timestamptz not null,
  created_at timestamptz not null default now(),
  primary key (withdrawal_id,attempt)
);

create index if not exists withdrawal_dispatch_attempt_claimed_idx
  on public.withdrawal_dispatch_attempt_events(withdrawal_id,claimed_at desc);

alter table public.withdrawal_dispatch_attempt_events enable row level security;

drop policy if exists "withdrawal_dispatch_attempt_no_public_access"
  on public.withdrawal_dispatch_attempt_events;
create policy "withdrawal_dispatch_attempt_no_public_access"
on public.withdrawal_dispatch_attempt_events
for all
...
revoke all on table public.withdrawal_dispatch_attempt_events
  from public,anon,authenticated,service_role;
grant select,insert on table public.withdrawal_dispatch_attempt_events
  to service_role;

create or replace function public.record_withdrawal_dispatch_attempt_event()
returns trigger
language plpgsql
security invoker
...
begin
  ...
  then
    insert into public.withdrawal_dispatch_attempt_events(
      withdrawal_id,attempt,claimed_at
    ) values (
      new.id,new.dispatch_attempts,new.dispatch_claimed_at
    )
...
revoke all on function public.record_withdrawal_dispatch_attempt_event()
  from public,anon,authenticated,service_role;

drop trigger if exists withdrawal_dispatch_attempt_event_guard
  on public.withdrawals;
create trigger withdrawal_dispatch_attempt_event_guard
after update of dispatch_attempts,dispatch_claimed_at
on public.withdrawals
for each row
execute function public.record_withdrawal_dispatch_attempt_event();

-- Backfill the latest known attempt for pre-existing rows. Earlier historical
-- attempts cannot be reconstructed and therefore are deliberately not invented.
insert into public.withdrawal_dispatch_attempt_events(
  withdrawal_id,attempt,claimed_at
)
select id,dispatch_attempts,dispatch_claimed_at
  ...
  if p_event_type='payout.sent' then
    ...
    if found then
      if trim(v_terminal.destination)=trim(p_destination)
         ...
         and exists (
           select 1
           from public.withdrawal_dispatch_attempt_events dae
           where dae.withdrawal_id=v_terminal.id
             and p_event_created_at between
               dae.claimed_at - interval '5 minutes'
               and dae.claimed_at + interval '15 minutes'
         )
      then
...
select
  ...
  and to_regclass('public.withdrawal_dispatch_attempt_events') is not null
  and coalesce((
    select relrowsecurity
    from pg_class
    where oid='public.withdrawal_dispatch_attempt_events'::regclass
  ),false)
  and exists (
    select 1 from pg_policy
    where polrelid='public.withdrawal_dispatch_attempt_events'::regclass
      and polname='withdrawal_dispatch_attempt_no_public_access'
  )
  and not has_table_privilege('anon','public.withdrawal_dispatch_attempt_events','SELECT')
  and not has_table_privilege('authenticated','public.withdrawal_dispatch_attempt_events','SELECT')
  and has_table_privilege('service_role','public.withdrawal_dispatch_attempt_events','SELECT')
  and exists (
    select 1 from pg_trigger
    where tgrelid='public.withdrawals'::regclass
      and tgname='withdrawal_dispatch_attempt_event_guard'
      and tgenabled <> 'D'
  )
  and position(
  ...
  and position(
    'dae.claimed_at-interval''5minutes''anddae.claimed_at+interval''15minutes'''
    in regexp_replace(
  ...
  and not exists (
    select 1
    from public.withdrawals w
    where w.dispatch_attempts > 0
      and w.dispatch_claimed_at is not null
      and not exists (
        select 1
        from public.withdrawal_dispatch_attempt_events dae
        where dae.withdrawal_id=w.id
          and dae.attempt=w.dispatch_attempts
          and dae.claimed_at=w.dispatch_claimed_at
      )
  )
  and coalesce((
    select
...
begin
  if not public.release_faucetpay_webhook_reconciliation_contract() then
    raise exception 'FaucetPay webhook causal binding contract failed';
  end if;
  ...
  if coalesce((
    ...
    raise exception 'FaucetPay webhook causal binding requires canonical release schema v55';
  end if;
end
$$;
