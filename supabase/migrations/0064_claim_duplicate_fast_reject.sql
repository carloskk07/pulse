-- Concurrency hardening: reject duplicate in-flight claims without waiting.
-- Compatible with release schema v55 / 0055. No reward, Treasury funding,
-- payout, RLS or release-authority changes are made.
--
-- v63 already compacted the daily-budget reads. This migration changes only
-- the per-user advisory acquisition from a blocking wait to a non-blocking
-- transaction-level try-lock. One transaction still owns the user claim key;
-- concurrent duplicates return claim_in_progress without consuming a backend
-- while waiting. Treasury serialization and all financial gates are unchanged.
...
begin
  ...
  if coalesce((v_schema->>'version')::integer, 0) <> 55
    ...
    raise exception 'claim duplicate fast-reject requires v55 authority';
  end if;
  ...
  if regexp_count(
     ...
     or position('pg_try_advisory_xact_lock' in v_claim_def) = 0
     or position('claim_in_progress' in v_claim_def) = 0
     or position('perform pg_advisory_xact_lock(' in v_claim_def) > 0
    ...
    raise exception 'claim duplicate fast-reject contract changed';
  end if;
  ...
  if not public.release_hourly_pulse_scale_contract() then
    raise exception 'hourly pulse scale contract failed after duplicate fast-reject';
  end if;
end
$$;
