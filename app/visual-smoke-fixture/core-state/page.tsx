...
type Props = { searchParams: Promise<{ scene?: string; event?: string; authority?: string }> };
...
export default async function DenseStateVisualFixture({ searchParams }: Props) {
  ...
  const baseExperience = experiences[scene];
  const experience: ProductExperience = params.authority === "none"
    ? {
      ...baseExperience,
      event: "none",
      residue: "none",
    ...
    : params.authority === "mismatch"
      ? {
        ...
        event: "none",
        residue: "network-active",
        residueDimension: scene === "progress" ? "network" : "signal",
        residueStrength: 100,
      }
      : baseExperience;
