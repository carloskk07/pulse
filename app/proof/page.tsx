  ...
  return (
    <main className="proof-page">
      ...
      <section className="proof-grid shell" aria-label="Pulse proof metrics">
        <article><small>Claims · 24h</small><strong>{proof.available ? proof.claims24h.toLocaleString("en-US") : "—"}</strong><span>treasury-backed Hourly Pulses</span></article>
        ...
        <article><small>Credited · all time</small><strong>{proof.available ? formatUsdFromCredits(proof.creditedAllTimeCredits) : "—"}</strong><span>Hourly Pulse rewards recorded</span></article>
      ...
      <section className="proof-principles shell">
        <article><Spark /><div><h2>Credited is not paid.</h2><p>Pulse distinguishes rewards added to the internal ledger from withdrawals completed by the payout provider.</p></div></article>
      ...
      <section className="proof-cta shell"><div><span className="section-kicker">The product loop</span><h2>Claim the base Pulse. Turbo only when you choose.</h2></div><Link className="button" href="/auth?next=/dashboard">Open Pulse <ArrowUpRight /></Link></section>
    </main>
  );
}
