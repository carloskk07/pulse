...
export default async function SupportPage({ searchParams }: Props) {
  ...
  return <main className="completion-page">
    ...
    <section className="completion-hero shell"><span className="section-kicker">Pulsercuit help center</span><h1>Tell us what happened. Keep one reference.</h1><p>Choose the issue and share what happened. We use your account history to investigate it.</p></section>
    <section className="completion-grid shell">
      ...
      <aside className="completion-card support-circuit-card"><span className="app-eyebrow">Helpful details</span><h2>What helps us resolve it faster.</h2><div className="faq-list"><details><summary>Missing faucet or extra reward</summary><p>Include the approximate time, the action you completed and any visible reference. We check the time and reference against your reward history.</p></details><details><summary>Payout issue</summary><p>Include the destination type and the status shown under Balance & payout. Do not create repeated payout attempts while one is processing.</p></details><details><summary>Privacy request</summary><p>Choose Privacy & data for access, correction, processing information, objection or deletion requests where applicable.</p></details></div><div className="support-proof-note"><i /> Screenshots help with context; reward and payout status are checked against your account records.</div></aside>
    </section>
