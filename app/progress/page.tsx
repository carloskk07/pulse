  ...
  return (
    <RoutePageTransition route="progress">
      ...
      <div className="app-page-head pc-progress-head pc-luxe-momentum-head">
        ...
        />
        <Link className="button pc-v5-primary" href={shareEntryHref} transitionTypes={productRouteTransitionTypes("progress", shareEntryHref)}>{shareReady ? "Share progress" : state.signedIn ? "View share status" : "Sign in"}</Link>
      </div>
      ...
      {shareReady ? (
        ...
        <section className="pc-progress-cta pc-luxe-momentum-cta pc-share-studio-placeholder" id="circuit-moments" aria-labelledby="share-studio-placeholder-title">
          ...
          <Link className="button button-lg pc-v5-primary" href={state.signedIn ? "/dashboard" : "/auth?next=/progress%23circuit-moments"} transitionTypes={productRouteTransitionTypes("progress", state.signedIn ? "/dashboard" : "/auth?next=/progress%23circuit-moments")}>{state.signedIn ? "View rewards" : "Sign in"} <ArrowUpRight /></Link>
        </section>
      )}
