...
requireText("lib/product-readiness.ts", ["PRODUCT_SETUP_CHECK_IDS", '"send-authority-config"', '"hourly-pulse-config"', '"treasury"', "getFaucetPaySendAuthorityConfig", '"faucetpay-send-scope-proof"', 'releaseEvidenceMatches(proof, "faucetpay_send_scope")', "hasProductSetupBlocker", "!item.pass"]);
...
requireText("lib/release-evidence.ts", ["FAUCETPAY_SEND_SCOPE_PROOF_SCHEMA", '"faucetpay_send_scope"', "process.env.FAUCETPAY_SEND_DAILY_LIMIT_USD", '"scope:send-only"', '"daily-cap:exact-value-operator-verified"', "GenericRecordableReleaseEvidenceKind", 'Exclude<ReleaseEvidenceKind, "faucetpay_payout">', "recordReleaseEvidence(kind: GenericRecordableReleaseEvidenceKind)"]);
